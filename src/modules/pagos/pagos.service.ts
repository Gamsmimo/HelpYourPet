  ...
  async createStripeCheckoutSession(dto: CreateStripeCheckoutDto): Promise<{ url: string; sessionId: string }> {
    if (!this.stripe) {
      throw new BadRequestException(
        'Stripe no está configurado correctamente. Define una STRIPE_SECRET_KEY válida (sk_test_...) y reinicia el backend.',
      );
    }
    ...
    try {
      ...
      throw new BadRequestException(
        `Stripe rechazó la sesión de pago: ${error?.message || 'Error desconocido'}`,
      );
    }
